...
Proof.
  intros f b.
  destruct (f true) eqn:EH.
  - destruct (f false) eqn:EH'.
    + destruct b eqn:EH''.
      * rewrite -> EH.
        rewrite -> EH.
        rewrite -> EH.
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)
   ...
   Proof: *)
   (*
     Let's perform induction on n.
     Base case (n = 0):
      We have (0 =? m) = (m =? 0). Let's case split on m.
        m = 0: (0 =? 0) = (0 =? 0)  holds by the definition of =?.
        m = S m': 0 =? (S m') = (S m') =? 0 holds by the definition of =?.
     Induction case (n = n'):
      Assume (n' =? m) = (m =? n') is true. Let's case split on n'.
        m = 0: (S n' =? 0) = (0 =? S n') holds by the definition of =?.
        m = S m': (S n' = S m') = (S m' = S n') follows (n' = m') = (m' = n')
                  by injection on constructor S and holds by induction hypothesis.
