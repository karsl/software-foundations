...
Check @combine : forall (X Y : Type), list X -> list Y -> list (prod X Y).
Check (combine [1;2] [false;false;true;true]) = [(1, false); (2, false)].
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  match l with
  | [] => ([], [])
  | (x, y) :: ps => (x :: (fst (split ps)), y :: (snd (split ps)))
  end.
...
Theorem map_dist : forall (X Y : Type) (f : X -> Y) (l1 l2 : list X),
...
Example fold_types_different1 :
  fold (fun n b => b && (0 <? n)) [2; 3; 4] true = true.
Proof. reflexivity. Qed.

Example fold_types_different2 :
  fold (fun n b => b && (0 <? n)) [2; 3; 4; 0] true = false.
Proof. reflexivity. Qed.
...
*)
(*
  Assumen we have a list l of type X with length n. Now, we must show
  that there's no element at index n of l.
  It's clear that the last element of l is at index n - 1, alternatively
  length of l minues one. Any index which is greater then n - 1 results
  in none. So, accessing nth element returns none since n > n - 1.
*)
