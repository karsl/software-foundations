...
(** **** Exercise: 1 star, standard, optional (destruct_induction) 
    ...
    In the context of natural numbers, both do case-split over constructors (case analysis).
    Induction tactic additional provides induction hypothesis for the induction step case.
...
(** **** Exercise: 2 stars, advanced, especially useful (plus_comm_informal) 
    ...
    Proof:
    I must show for any natural numbers n and m, n + m = m + n.
    Let's do induction on n.
    Base case (n = 0): 0 + m = m + 0 <=> m = m + 0 and m + 0 = m
                       by plus_n_O (I'll not prove this as this is already proven formally).
    Indution step (n = S n'): Assume n' + m = m + n' holds (Induction hypothesis).
                              We must prove (S n') + m = m + (S n').
                              Applying function addition once on the LHS of equality we get
                              S (n' + m) = m + (S n').
                              By induction hypothesis we get, S (m + n') = m + (S n').
                              By plus_n_Sm we get m + (S n') = m + (S n').
*)
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal) 
    ...
    Proof: 
    By induction on n:
    - Base case (n = 0): 0 =? 0 = true trivially.
    - Induction step (n = S n'): Induction hypothesis (IH) is true = n' =? n'.
      We must show true = S n' =? S n'.
      Applying function =? for one with params (S n') and (S n'), we get true = n' =? n' which is true by IH.
*)
(** [] *)
...
(* Problem is in the case when the binary number Z.
   There is no corresponding natural number to binary number Z. *)
...
Example normalizeTest1 : normalize (B0 Z) = B0 Z.
  reflexivity. Qed.
Example normalizeTest2 : normalize (B1 (B0 Z)) = B1 (B0 Z).
  reflexivity. Qed.
